...
namespace MvcEntityFramework.Models
{
    ...
    #region 模型
    public class StudentInfo//获取当前用户的信息
    {
        public static student getInfo { get;set;}
    }
    ...
    public interface IMembershipService
    {
        ...
        bool ValidateSuperUser(string userName, string password);//add
    ...
    public class AccountMembershipService : IMembershipService
    {
        ...
        public bool ValidateUser(string userName, string password)
        {
            ...
            if (q.Count() > 0)
            {
                StudentInfo.getInfo = new student();
                StudentInfo.getInfo.userName = userName;
                StudentInfo.getInfo.stuId = q.First().stuId;
                return true;
            }
            else
            {
        ...
        public bool ValidateSuperUser(string userName, string password)
        {
            ...
            var q = (from m in db.SuperUser
                     where m.Name == userName && m.Password == password
                     select m);
        ...
        public MembershipCreateStatus CreateUser(string userName, string password, string email)
        {
            ...
            if (stu.Count() == 0)//将用户信息写入数据库
            {
                _provider.CreateUser(userName, password, /*email,*/null, null, null, true, null, out status);
                student stuToCreate = new student();
                stuToCreate.userName = userName;
                stuToCreate.passWord = password;
